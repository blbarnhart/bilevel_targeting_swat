...
!!      read scheduled management practices
     ...
     &                                    frt_surface(iro,ifn,ihru) = .2
              ...
              !! barnhart fertilizer is subject to reduction policy
              !!idplt(iro,npl,ihru)
              if (ifrttyp(iro,ifn,ihru) == 3) then 
                if (frt_kg_pga(ihru) < 990) then
			!frt_kg(iro,ifn,ihru) = mgt4 * frt_kg_pga(ihru)
			frt_kg(iro,ifn,ihru) = frt_kg_pga(ihru)
		end if
              end if
               !!!! link from GAMS, split fertilizer application for RYEG   
!              if (ifn == 1) frt_kg(iro,ifn,ihru) = 0.3 * frt_kg_pga(i)
!              if (ifn == 2) frt_kg(iro,ifn,ihru) = 0.7 * frt_kg_pga(i)              
...
!!  added line below for pesticide 3/31/08 gsm
          ...
          case (6)  !! tillage operation
            ...
            cnop(iro,icnp,ihru) = mgt4
            !barnhart
            if (tillchoice_pga(ihru) > 0.5) then
              idtill(iro,inop,ihru) = 4
              cnop(iro,icnp,ihru) = cn2(ihru)-3
            endif
            icnp = icnp + 1
