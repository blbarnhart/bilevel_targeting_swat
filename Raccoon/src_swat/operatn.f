      ...
      if (igro(j) == 0) then
        ...
        if (iplant(nro(j),icr(j)+1,j) > 0) then
          if (iida == iplant(nro(j),icr(j)+1,j)) then
            call plantop
            write (124,1000) j, iyr, i_mo, iida, "     PLANT"
          end if
...
!! operations performed only when land cover is growing
     ...
     &                                           phuacc(j) > 0.75)  then
          select case (idc(idplt(nro(j),icr(j),j)))
            case (1, 4, 5)
              call harvkillop
              write (124,1000) j, iyr, i_mo, iida, "  END GROW"
          end select
        end if
        ...
        if (ihv(nro(j),icr(j),j) > 0) then
          if (iida == ihv(nro(j),icr(j),j)) then
            call harvkillop
            write (124,1000) j, iyr, i_mo, iida, " HARV&KILL"
          end if
        else
        ...
        if (ihvo(nro(j),ncut(j),j) > 0) then
          if (iida == ihvo(nro(j),ncut(j),j)) then
            if (ihv_gbm(nro(j),ncut(j),j) == 0) then
              call harvestop
	      write (124,1000) j, iyr, i_mo, iida, "   HARVEST"
            else 
              call harvgrainop
            end if 
          else
        ...
        if (ikill(nro(j),icr(j),j) > 0) then
          if (iida == ikill(nro(j),icr(j),j)) then
            call killop
            write (124,1000) j, iyr, i_mo, iida, "      KILL"
          end if
        else
...
!! operations performed at any time
      ...
      do while (iida == iop(nro(j),ntil(j),j))
        call newtillmix(j,0.)
        write (124,1000) j, iyr, i_mo, iida, "   TILLAGE"
      end do
