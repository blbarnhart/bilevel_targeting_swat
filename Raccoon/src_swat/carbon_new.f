      ...
      !! The code was developed by Armen R. Kemanian in collaboration with Stefan Julich
      !! It has been adapted from Kemanian and Stockle (200X) European Journal of Agronomy
    ...
    !! initialize
     ...
     &     * 1000. * (1- sol_rock(k,j) / 100.)
      ...
      sum_n_i = sol_nmass + sol_no3(k,j) + sol_fon(k,j) + sol_mn(k,j)
     ...
     &    + sol_mp(k,j)
      ...
      if (sol_tmp(kk,j) > 0. .and. sol_st(kk,j) > 0.) then
        ...
        sol_cdec = fsol_cdec(sol_cbn(k,j),cx,tilf,csf,sol_cmass)
        ...
        if (sol_rsd(k,j) > 0.00001) then
            ...
            !! humification factor
            rhc = fhc(sol_clay(k,j),sol_cbn(k,j),cx)
        ...
        if (sol_mc(k,j) > 0.00001) then
            ...
            !! humification factor
            mhc = 1.8 * fhc(sol_clay(k,j),sol_cbn(k,j),cx)
    ...
    !! balance file cswat_balance
     ...
     &    + (1. - rhc) * rdc * 0.43 + sol_mc(k,j) + (1. - mhc) * mdc
      sum_n_f = sol_nmass + sol_no3(k,j) + sol_fon(k,j) + sol_mn(k,j) 
     &    + wdn
    ...
    !! writing daily output by layer
     ...
     &    sol_cmass/sol_nmass, sol_nmass/sol_orgp(k,j)
    ...
    !! writing daily profile output
      if (j==1) then
        write (100,9002) iyr, i, j, cmass_pro, nmass_pro,  
     &     sol_rsd_pro, sol_cdec_pro, sol_fon_pro, 
     &     sol_no3_pro, sol_orgp_pro, sol_fop_pro, sol_solp_pro,
     &     sol_mc_pro, sol_mn_pro, sol_mp_pro
      end if

9000  format(i4,';',i3,';',i1,';',i4,16(';',f10.3))
9001  format(i4,';',i3,';',i1,';',i4,10(';',f10.3))
9002  format(i4,';',i3,';',i4,';',f11.3,';',f11.3,13(';',f10.3))
