...
!       if (ipot(j) /= j .and. imp_trig(nro(j),nrelease(j),j)==1)       &
     ...
     &    smx(j)) - precipday + qday + latq(j) + sepbtm(j) + qtile
        ...
        do while (iida == ifert(nro(j),nfert(j),j))
          call fert
          write (124,1000) j, iyr, i_mo, iida, "  FERT APP"
        end do
...
!$$$$$$         end if
        ...
        do while (iida == ipst(nro(j),npest(j),j))
          call apply
          write (124,1000) j, iyr, i_mo, iida, "  PEST APP"
        end do
        if (igro(j) == 0) then
          aphu = phubase(j)
...
!$$$$$$         end if
        ...
        if (iida == irelease(nro(j),nrelease(j),j)) then
          ...
          write (124,1000) j, iyr, i_mo, iida, "   RELEASE"
          ...
          if (irrsc(j) > 2) then
            ...
            if (iir(nro(j),nirr(j),j) > 0) then
              if (iida == iir(nro(j),nirr(j),j)) then
                call irrsub
                write (124,1000) j, iyr, i_mo, iida, "  IRRIGATE"
            end if
            else
