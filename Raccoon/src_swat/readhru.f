...
!         slsubbsn(ihru) = slsubbsn(ihru)*slsubbsn_pga(ihru)
         
      read (108,*) hru_slp(ihru) 
!         hru_slp(ihru) = hru_slp(ihru)*hruslp_pga(ihru)
         
      read (108,*) ov_n(ihru)
!         ov_n(ihru) = ov_n(ihru)*ovn_pga(ihru)

      read (108,*) lat_ttime(ihru) 
      read (108,*) lat_sed(ihru)   !read in in mg/L
      read (108,*) slsoil(ihru)
!        slsoil(ihru) = slsoil_pga(ihru) 

      read (108,*,iostat=eof) canmx(ihru) 
!        canmx(ihru) = canmx_pga(ihru)        
      if (eof < 0) exit
      
      read (108,*,iostat=eof) escohru
!        escohru = esco_pga(ihru)
      if (eof < 0) exit
      
      read (108,*,iostat=eof) epcohru 
!         epcohru = epco_pga(ihru)
      if (eof < 0) exit
