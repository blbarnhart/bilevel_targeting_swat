...
!!    rchmono(p12,:)    |kg         |conservative metal #1 transported out of
!!                                 |reach during month
...
c      if (jrch .eq. 108) then 
c	 rchdy2_pga(1,dtot) = rchdy(2,jrch)   !! flow out (m^3/s)
c!         print *, "rchdy2_pga = ",rchdy2_pga(dtot), dtot
c      end if 
...
!! set outputs for Malmquist index
      !!Barnhart GA get output.
      if (jrch .eq. 108) then
c        swatout_ga(dtot,1) = rchdy(2,jrch) !flow out
c        swatout_ga(dtot,2) = rchdy(6,jrch) !sedOut tons
c        swatout_ga(dtot,3) = rchdy(7,jrch) !sedimentCONC

c         rchdy2_pga(1,dtot) = rchdy(13,jrch)
        rchdy2_pga(1,dtot) = rchdy(9,jrch)+ !TotN = orgN+NO3+NH4+NO2
     &    rchdy(13,jrch) + !NO3
     &    rchdy(15,jrch) + !NH4
     &    rchdy(17,jrch) !NO2

c        swatout_ga(dtot,5) = rchdy(11,jrch) + rchdy(19,jrch) !!Tot P=orgP+solP
c        swatout_ga(dtot,6) = rchdy(9,jrch) !orgN
c        swatout_ga(dtot,7) = rchdy(13,jrch) !NO3
c        swatout_ga(dtot,8) = rchdy(15,jrch) !NH4
c        swatout_ga(dtot,9) = rchdy(17,jrch) !NO2
c        swatout_ga(dtot,10) = rchdy(11,jrch) !orgP
c        swatout_ga(dtot,11) = rchdy(19,jrch) !solP
c        swatout_ga(dtot,12) = rchdy(23,jrch) !CBOD kg
c        swatout_ga(dtot,13) = rchdy(25,jrch) !dissolvedO2 kg
c        swatout_ga(dtot,14) = rchdy(60,jrch) !stream temp
       !print *, dtot
      end if


c      if (jrch .eq. 108) then 
c        rchdy2_pga(1,dtot) = rchdy(9,jrch) + !orgN
c    &       rchdy(13,jrch) + !no3
c    &       rchdy(15,jrch) + !nh4
c    &       rchdy(17,jrch)   !no2
c        !rchdy2_pga(2,dtot) = rchdy(7,jrch)    !!sediment concentration in outflow
c        !rchdy2_pga(3,dtot) = rchdy(9,jrch)    !!organic N transported out of reach on day
c        !rchdy2_pga(4,dtot) = rchdy(11,jrch)   !!organic P transported out of reach on day
c        !rchdy2_pga(5,dtot) = rchdy(13,jrch)   !!nitrate transported out of reach on day
c        !rchdy2_pga(6,dtot) = rchdy(15,jrch)   !!ammonia transported out of reach on day
c        !rchdy2_pga(7,dtot) = rchdy(17,jrch)   !!nitrite transported out of reach on day
c        !rchdy2_pga(8,dtot) = rchdy(19,jrch)   !!soluble P transported out of reach on day  !!solP out (kg P)
c         
c!         print *, "rchdy2_pga = ",rchdy2_pga(dtot), dtot
c      end if 
