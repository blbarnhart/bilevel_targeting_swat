...
#	if ((!is.na(swat_input[j])))  {
		...
		ycorn_np[j] = qvaluesused[j]*((a10*gam11^(dc*s2nopolicy)*gam12^(dc*s3nopolicy)*gam13^((1-dc)*s2nopolicy)*gam14^((1-dc)*s3nopolicy)) + 
...
#	if ((!is.na(swat_input[j])))  {
		dc = 0; #since last crop was SOYB
		ycorn_wp[j] = qvaluesused[j]*((a10*gam11^(dc*s2policy[j])*gam12^(dc*s3policy[j])*gam13^((1-dc)*s2policy[j])*gam14^((1-dc)*s3policy[j])) + 
			a11*dc + a12*(swat_input[j]-(row*dc)) + a13*((swat_input[j]-row*dc)^2))
...
colnames(dfout) <- c("swat_input_kg_ha","swat_input_till","indivtaxObj","taxObj","tprofit_wp","tprofit_np","profit_diffs","basinprofit_diffs","tax",
					"q","sub","area_acres",
...
#input_taxes_tills = c(input_taxes,rep(0.1,112))
	input_tills = as.numeric(inputs[i,3:114])
	input_taxes_tills = c(rep(1.0,112),input_tills)
...
write.csv(out4,'hruresults_nopolicy_1.csv',quote=F,row.names=F)
write.csv(out4,'hruresults_taxtill_29.csv',quote=F,row.names=F)
write.csv(out4,'hruresults_taxonly_62.csv',quote=F,row.names=F)
write.csv(out4,'hruresults_tillonly_42.csv',quote=F,row.names=F)

write.csv(out4,'hruresults_maxNreductaxonly_taxonly_4.csv',quote=F,row.names=F)
write.csv(out4,'hruresults_maxNreductaxonly_tillonly_15.csv',quote=F,row.names=F)
write.csv(out4,'hruresults_maxNreductillonly_tillonly_6.csv',quote=F,row.names=F)
write.csv(out4,'hruresults_maxNreductillonly_taxtill_21.csv',quote=F,row.names=F)
write.csv(out4,'hruresults_maxNreductaxtill_taxtill_83.csv',quote=F,row.names=F)
